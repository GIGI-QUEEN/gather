...
func ChangeCommentLike(commentId int, userId int) error {
	row := DB.QueryRow("select user_id from comment_likes where (comment_id, user_id)=(?,?)", commentId, userId)
	id := -1
	err := row.Scan(&id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			_, err := DB.Exec("delete from comment_dislikes where (comment_id,user_id) = (?,?)", commentId, userId)
			if err != nil && err != sql.ErrNoRows {
				return err
			}
		...
		_, err := DB.Exec("delete from comment_likes where (comment_id,user_id) = (?,?)", commentId, userId)
		if err != nil {
			return err
		}
	}
...
func ChangeCommentDisLike(commentId int, userId int) error {
	row := DB.QueryRow("select user_id from comment_dislikes where (comment_id, user_id)=(?,?)", commentId, userId)
	id := -1
	err := row.Scan(&id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			...
			_, err = DB.Exec("insert into comment_dislikes (comment_id, user_id) values (?,?)", commentId, userId)
			if err != nil {
				return err
			}
		} else {
